...
Module_Exports({
		...
		if (!text) return await citel.reply("Enter a tg sticker url_l\nExample: .tgs https://t.me/addstickers/Oldboyfinal\nKeep in mind that there is a chance of ban if used always.");
...
Module_Exports({
    ...
    if (apkSize > 150) {
      return person.reply("File size bigger than 200MB.");
    }
    ...
    axios.get(url, {
      ...
      fs.unlink(filePath);
      return person.reply("Apk not Found, Sorry");
    });
  });
  //---------------------------------------------------------------------------
Module_Exports({
...
async(sigma, person, memo) => {
    ...
    if (!tax) return citel.send(`Example: ${prefix}video Moon Men`);
    ...
    if (!tax) return citel.send(`Example: ${prefix}song Moon Men`);
        ...
        if (fileSizeInMegabytes <= dlsize) {
            ...
            citel.reply(`File Size Is Bigger Than 100MB.`);
        }
...
`;
 ...
 return await Void.sendMessage(citel.chat, buttonMessaged)
    ...
    async(sigma, citel, tax) => {
        let yts = require("secktor-pack");
    if (!tax) return citel.send(`Example: ${prefix}audio Moon Men`);
        let search = await yts(tax);
        let anu = search.videos[0];
        const getRandom = (ext) => {
            return `${Math.floor(Math.random() * 10000)}${ext}`;
